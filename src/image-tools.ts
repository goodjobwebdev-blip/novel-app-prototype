...
export function executeImageProposal(call: ChatToolCall): { content: string; imageGeneration?: ChatImageProposal } {
  try {
    ...
    return { imageGeneration: proposal, content: JSON.stringify({ ok: true, proposalId: proposal.id, task, message: 'Visual proposal created. No media has been generated. The user must accept the proposal, then press Generate.' }) }
