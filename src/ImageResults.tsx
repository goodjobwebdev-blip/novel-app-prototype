...
import { cancelImageJob, decideImageJob, getGalleryImage, hideImageFromChat, listImageJobs, retryImageJob } from './image-store'
...
function JobResult({ job, chat }: { job: ImageJob; chat: boolean }) {
  ...
  return <div className="image-job-result">{asset ? <ImageAssetPreview asset={asset} /> : <p>{loadError || 'Loading image…'}</p>}<div className="image-actions">{!job.decision && asset && <><button disabled={busy} type="button" onClick={() => { void action(() => decideImageJob(job.id, true)) }}>Keep image</button><button disabled={busy} type="button" onClick={() => { void action(() => decideImageJob(job.id, false)) }}>Discard</button></>}{job.decision === 'kept' && <span>Saved to gallery</span>}{chat && <button disabled={busy} type="button" onClick={() => { void action(() => hideImageFromChat(job.id)) }}><Trash2 size={16} /> Remove from chat</button>}</div>{error && <p role="alert">{error}</p>}</div>
}
export default function ImageJobs({ proposalId, messageId }: { proposalId?: string; messageId?: string }) {
  ...
  const matching = jobs.filter((j) => !proposalId || (j.proposalId === proposalId && j.messageId === messageId && !j.hiddenInChat))
  ...
  return <div className="image-jobs">{(error || actionError) && <p role="alert">{error || actionError}</p>}{!visible.length && !proposalId && <p className="image-help">Your generation queue is empty.</p>}{visible.map((job) => <article className="image-job" key={job.id}><header><strong>{job.modelAlias}</strong><span>{job.status === 'running' ? `Generating · ${Math.max(0, Math.floor((now - (job.startedAt ?? now)) / 1000))} s` : job.status === 'queued' ? `Queued · #${jobs.filter((j) => j.provider === job.provider && j.status === 'queued').findIndex((j) => j.id === job.id) + 1}` : job.status}</span></header><small>{job.size.width} × {job.size.height}{!proposalId && job.bookTitle ? ` · ${job.bookTitle}` : ''}</small>{!proposalId && <p className="image-prompt-preview">{job.prompt}</p>}{['queued', 'running'].includes(job.status) && <button type="button" onClick={() => action(() => cancelImageJob(job.id))}>{job.status === 'running' ? 'Stop waiting' : 'Cancel queued image'}</button>}{job.status === 'running' && <small>Stopping does not guarantee the provider cancels its charge.</small>}{job.error && <p role="alert">{job.error}</p>}{['failed', 'interrupted', 'cancelled'].includes(job.status) && <button type="button" onClick={() => action(() => retryImageJob(job.id))}>{job.providerJobId ? 'Check existing result' : 'Retry as new generation'}</button>}{job.status === 'completed' && <JobResult job={job} chat={Boolean(proposalId)} />}</article>)}{visible.length < matching.length && <button type="button" onClick={() => setLimit(limit + 30)}>Show earlier generations</button>}</div>
}
