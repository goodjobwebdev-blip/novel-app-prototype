import { THINKING_EFFORT_OPTIONS, normalizeThinkingEffort, type ThinkingEffort } from '../../shared/ai/thinking-effort'
...
import { ChatImageReferences, CharacterChatSetup, CharacterPosition } from './CharacterChatControls'
import CharacterMessage from './CharacterMessage'
import ImageJobs from '../images/ImageResults'
import { executeCodexCutoffRead } from './chat-codex-cutoff'
import type { LoreType } from '../codex/lore-types'
...
import { projectProse } from '../editor/document-projection'
...
import { bookTemplateValues, promptTemplateDiagnostics, promptVariables, type BookPromptValues } from '../../shared/ai/prompt-template'
...
import {
  ...
  chatRequestValues, emptyCharacterBook,
...
import { clonePromptComposition, likelyReusablePrefix, makePredefinedMessage, normalizeRuntimeMessagePart, type NormalizedAssembledRequest, type NormalizedRequestPart, type PredefinedMessage, type PromptComposition } from '../../shared/ai/prompt-composition'
import { startTtsSession } from '../speech/tts-service'
import { cancelSttSession, stopSttSession, getSttState, normalizeTranscriptForInsertion, startSttSession, subscribeSttState, type SttState } from '../speech/stt-service'
import './chat.css'
...
export function ChatView({ bookId, chatId, bookPromptValues, currentSceneId, onChatChange, onToast }: ChatViewProps) {
  ...
  function buildNormalizedRequest(
    ...
    return assembleChatGenerationRequest({
      ...
      history,
      restrictedInstructions: prepared.character?.instructions,
      tools: availableChatTools(Boolean(activeChat.character), history),
    })
  }
  ...
  async function prepareAssistantGeneration(
    ...
    const instructionsWarning = activeChat.character ? '' : chatWorkspaceInstructionsWarning(activeChat.promptComposition)
    if (instructionsWarning) onToast(instructionsWarning)

    let character: CharacterFrame | undefined
    let loreTypes: LoreType[]
    let skills: CapturedChatSkill[]
    let context: Awaited<ReturnType<typeof buildContextValues>>
    try {
      if (activeChat.character) { character = await captureCharacterFrame(activeChat, history); context = character.context; skills = []; loreTypes = [] }
      else ({ context, skills, loreTypes } = await prepareChatSkillContext(activeChat, currentSceneId || undefined))
    } catch (error) {
      assertGenerationOwnerCurrent(owner, activeChat)
  ...
  return <>
    <section className="conversation functional-chat">
      <header><small>{chat.character ? 'Character chat' : 'Book chat'}</small><h1>{chat.title}</h1></header>
